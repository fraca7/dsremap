...
USBController::USBController(USB* pUsb, Host* pHost)
  : HIDUniversal(pUsb),
    m_pHost(pHost),
    m_Interface(0xFF),
...
uint8_t USBController::OnInitSuccessful()
{
...
void USBController::ParseHIDData(USBHID* pHID, bool is_rpt_id, uint8_t len, uint8_t* buf)
{
...
void USBController::EndpointXtract(uint8_t conf, uint8_t iface, uint8_t alt, uint8_t proto, const USB_ENDPOINT_DESCRIPTOR *pep)
{
  if (m_Interface == 0xFF) {
    m_Interface = iface;
...
void USBController::GET_REPORT(uint8_t reportType, uint8_t reportId, uint16_t len)
{
...
void USBController::SET_REPORT(uint8_t reportType, uint8_t reportId, uint16_t len, const uint8_t* buf)
{
...
void USBController::SendData(uint16_t len, const uint8_t* buf)
{
