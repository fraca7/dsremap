...
class TestCast(unittest.TestCase):
    ...
    def test_regoff_int_regoff_float(self):
        ...
        vm, size = self.create(stack=struct.pack('<hhf', 1, 13, 42.5))
        vm.step(Report())

        self.assertEqual(vm.get_stack(), struct.pack('<hhf', 1, 42, 42.5))
        self.assertEqual(vm.offset, size)

    def test_regoff_float_regoff_int(self):
        self.add_opcode()
        self.add(Opcodes.make_addr_regoff(Opcodes.REGINDEX_ZR, 2, Opcodes.ADDR_VALTYPE_FLOAT))
        self.add(Opcodes.make_addr_regoff(Opcodes.REGINDEX_ZR, 6, Opcodes.ADDR_VALTYPE_INT))

        vm, size = self.create(stack=struct.pack('<hfh', 1, 42.5, 13))
        vm.step(Report())

        self.assertEqual(vm.get_stack(), struct.pack('<hfh', 1, 13.0, 13))
        self.assertEqual(vm.offset, size)
    ...
    def test_reg_int_regoff_float(self):
        ...
        vm, size = self.create(stack=struct.pack('<hf', 1, 42.5))
        vm.step(Report())
...
class TestAdd(unittest.TestCase):
    ...
    def test_constaddr_C_int(self):
        self.add_opcode(Opcodes.OPCODE_VARIANT_C)
        self.add(Opcodes.make_addr_regoff(Opcodes.REGINDEX_ZR, 5, Opcodes.ADDR_VALTYPE_INT))
        self.add(struct.pack('<h', 42))

        vm, size = self.create(stack=struct.pack('<hhbhh', 1, 2, 3, 4, 5))
        vm.step(Report())

        self.assertEqual(vm.get_stack(), struct.pack('<hhbhh', 1, 2, 3, 4 + 42, 5))
        self.assertEqual(vm.offset, size)

    def test_constaddr_C_float(self):
        self.add_opcode(Opcodes.OPCODE_VARIANT_C)
        self.add(Opcodes.make_addr_regoff(Opcodes.REGINDEX_ZR, 5, Opcodes.ADDR_VALTYPE_FLOAT))
        self.add(struct.pack('<f', 42.5))

        vm, size = self.create(stack=struct.pack('<hhbfh', 1, 2, 3, 4.0, 5))
        vm.step(Report())

        self.assertEqual(vm.get_stack(), struct.pack('<hhbfh', 1, 2, 3, 4.0 + 42.5, 5))
        self.assertEqual(vm.offset, size)
    ...
    def test_regoff_C_int(self):
        ...
        self.assertEqual(vm.get_stack(), struct.pack('<hhh', 1, 2 + 42, 3))
        self.assertEqual(vm.offset, size)
    ...
    def test_regoff_C_float(self):
        ...
        self.assertEqual(vm.get_stack(), struct.pack('<hfh', 1, 2.5 + 42.5, 3))
        self.assertEqual(vm.offset, size)
    ...
    def test_constaddr_constaddr_int(self):
        ...
        vm, size = self.create(stack=struct.pack('<hhh', 1, 13, 42))
        vm.step(Report())

        self.assertEqual(vm.get_stack(), struct.pack('<hhh', 1, 13 + 42, 42))
        self.assertEqual(vm.offset, size)

    def test_constaddr_constaddr_float(self):
        self.add_opcode(Opcodes.OPCODE_VARIANT_A)
        self.add(Opcodes.make_addr_regoff(Opcodes.REGINDEX_ZR, 2, Opcodes.ADDR_VALTYPE_FLOAT))
        self.add(Opcodes.make_addr_regoff(Opcodes.REGINDEX_ZR, 6, Opcodes.ADDR_VALTYPE_FLOAT))

        vm, size = self.create(stack=struct.pack('<hff', 1, 13.5, 42.5))
        vm.step(Report())

        self.assertEqual(vm.get_stack(), struct.pack('<hff', 1, 13.5 + 42.5, 42.5))
        self.assertEqual(vm.offset, size)

    def test_regoff_regoff_int(self):
        self.add_opcode(Opcodes.OPCODE_VARIANT_A)
        self.add(Opcodes.make_addr_regoff(Opcodes.REGINDEX_SP, -4, Opcodes.ADDR_VALTYPE_INT))
        self.add(Opcodes.make_addr_regoff(Opcodes.REGINDEX_SP, -6, Opcodes.ADDR_VALTYPE_INT))

        vm, size = self.create(stack=struct.pack('<hhhh', 1, 2, 3, 4))
        vm.step(Report())

        self.assertEqual(vm.get_stack(), struct.pack('<hhhh', 1, 2, 5, 4))
        self.assertEqual(vm.offset, size)

    def test_regoff_regoff_float(self):
        self.add_opcode(Opcodes.OPCODE_VARIANT_A)
        self.add(Opcodes.make_addr_regoff(Opcodes.REGINDEX_SP, -6, Opcodes.ADDR_VALTYPE_FLOAT))
        self.add(Opcodes.make_addr_regoff(Opcodes.REGINDEX_SP, -10, Opcodes.ADDR_VALTYPE_FLOAT))

        vm, size = self.create(stack=struct.pack('<hffh', 1, 2.5, 3.5, 4))
        vm.step(Report())

        self.assertEqual(vm.get_stack(), struct.pack('<hffh', 1, 2.5, 6.0, 4))
        self.assertEqual(vm.offset, size)
    ...
    def test_reg_constaddr(self):
        ...
        vm, size = self.create(stack=struct.pack('<hhbhh', 1, 2, 3, 42, 13))
        vm.step(Report())

        self.assertEqual(vm.SP, 9 + 42)
        self.assertEqual(vm.offset, size)
    ...
    def test_constaddr_reg(self):
        ...
        vm, size = self.create(stack=struct.pack('<hh', 1, 2))
        vm.TH = 42;
        vm.step(Report())

        self.assertEqual(vm.get_stack(), struct.pack('<hh', 1, 2 + 42))
        self.assertEqual(vm.offset, size)
    ...
    def test_constaddr_report_float(self):
        ...
        vm, size = self.create(stack=struct.pack('<hf', 1, 2.0))
        report = Report(IMUX=42.5)
        vm.step(report)

        self.assertEqual(vm.get_stack(), struct.pack('<hf', 1, 2.0 + 42.5))
        self.assertEqual(vm.offset, size)
    ...
    def test_constaddr_regoff_int(self):
        ...
        self.assertEqual(vm.get_stack(), struct.pack('<hhhh', 1, 5, 3, 4))
        self.assertEqual(vm.offset, size)
    ...
    def test_constaddr_regoff_float(self):
        ...
        self.assertEqual(vm.get_stack(), struct.pack('<hffh', 1, 6.0, 3.5, 4))
        self.assertEqual(vm.offset, size)
